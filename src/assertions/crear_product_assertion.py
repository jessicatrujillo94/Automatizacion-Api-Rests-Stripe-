...
def assert_creacion_fallida(response):
    try:
        ...
        pytest.xfail(f"XFAIL por fallo en creación: {e}")
    except Exception as e:
        pytest.xfail(f"XFAIL por excepción inesperada: {e}")
...
def assert_creacion_permisos_insuficientes(response):
    try:
...
def assert_creacion_sin_campo_obligatorio(response):
    try:
        assert response.status_code == 400, f"Se esperaba 400, se recibió {response.status_code}"
    except AssertionError as e:
        pytest.xfail(f"XFAIL campo obligatorio ausente: {e}")
    except Exception as e:
        pytest.xfail(f"XFAIL por excepción inesperada: {e}")
...
def assert_creacion_caracteres_especiales(response):
    try:
        assert response.status_code in [200, 201], f"Se esperaba 200 o 201, se recibió {response.status_code}"
    except AssertionError as e:
        pytest.xfail(f"XFAIL caracteres especiales: {e}")
    except Exception as e:
        pytest.xfail(f"XFAIL por excepción inesperada: {e}")

def assert_estructura_respuesta_producto(response):
    try:
        assert response.status_code in [200, 201], f"Código inesperado: {response.status_code}"
        jsonschema.validate(instance=response.json(), schema=product_output_schema)
    except (AssertionError, jsonschema.ValidationError) as e:
        pytest.xfail(f"XFAIL estructura de producto inválida: {e}")
    except Exception as e:
        pytest.xfail(f"XFAIL por excepción inesperada: {e}")
...
def assert_creacion_nombre_largo(response):
    try:
        ...
        pytest.xfail(f"XFAIL nombre largo: {e}")
    except Exception as e:
        pytest.xfail(f"XFAIL por excepción inesperada: {e}")
...
def assert_creacion_inactivo(response):
    try:
        assert response.status_code in [200, 201], f"Se esperaba 200 o 201, se recibió {response.status_code}"
        data = response.json()
        assert data.get("active") is False, "El producto no está inactivo"
    except AssertionError as e:
        pytest.xfail(f"XFAIL producto inactivo: {e}")
    except Exception as e:
        pytest.xfail(f"XFAIL por excepción inesperada: {e}")
