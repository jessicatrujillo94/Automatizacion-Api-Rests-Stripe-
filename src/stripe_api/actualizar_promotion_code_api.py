...
def actualizar_promo_active_false(promotion_code):
    promotion_code_id = promotion_code.get("id")
    headers = build_headers()
    payload = {"active": False}
    endpoint = PromotionCodeEndpoints.UPDATE_PROMOTION_CODE.value.format(promotion_code_id=promotion_code_id)
...
def actualizar_promo_restriction_usage(promotion_code, usage_limit=5):
    promotion_code_id = promotion_code.get("id")
    headers = build_headers()
...
def actualizar_promo_id_invalido():
    headers = build_headers()
    promotion_code_id = "promo_invalido_123"
    payload = {"active": True}
    endpoint = PromotionCodeEndpoints.UPDATE_PROMOTION_CODE.value.format(promotion_code_id=promotion_code_id)
    response = StripeAPI(BASE_URL).post(endpoint=endpoint, headers=headers, payload=payload)
    log_request_response(url=f"{BASE_URL}{endpoint}", headers=headers, payload=payload, response=response)
...
def actualizar_promo_sin_token(promotion_code):
    promotion_code_id = promotion_code.get("id")
...
def actualizar_promo_token_invalido(promotion_code):
    promotion_code_id = promotion_code.get("id")
...
def actualizar_promo_body_vacio(promotion_code):
    promotion_code_id = promotion_code.get("id")
    headers = build_headers()
    payload = {}
    endpoint = PromotionCodeEndpoints.UPDATE_PROMOTION_CODE.value.format(promotion_code_id=promotion_code_id)
...
def actualizar_promo_active_invalido(promotion_code):
    promotion_code_id = promotion_code.get("id")
    headers = build_headers()
    payload = {"active": "yes"}
    endpoint = PromotionCodeEndpoints.UPDATE_PROMOTION_CODE.value.format(promotion_code_id=promotion_code_id)
...
def actualizar_promo_restriction_invalido(promotion_code):
    promotion_code_id = promotion_code.get("id")
    headers = build_headers()
...
def actualizar_promo_multiple_campos(promotion_code):
    promotion_code_id = promotion_code.get("id")
    headers = build_headers()
    payload = {
        "active": True,
