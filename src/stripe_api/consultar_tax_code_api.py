...
def consultar_tax_code(tax_code):
...
def consultar_tax_code_sin_token(tax_code):
    tax_code_id = tax_code.get("id", "")
...
def consultar_tax_code_token_invalido(tax_code):
    tax_code_id = tax_code.get("id")
...
def consultar_tax_code_case_sensitive():
    tax_code_id = "TX_1"
    headers = build_headers()
    endpoint = TaxCodeEndpoints.GET_TAX_CODE_BY_ID.value.format(tax_code_id=tax_code_id)
    response = StripeAPI(BASE_URL).get(endpoint=endpoint, headers=headers)
