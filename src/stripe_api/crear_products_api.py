...
def crear_producto_permisos_insuficientes():
    ...
    response = StripeAPI(BASE_URL).post(endpoint="/v1/restricted_resource", headers=headers, payload=payload)
...
def crear_multiples_productos():
    ...
    for producto in productos:
        response = StripeAPI(BASE_URL).post(endpoint=ENDPOINT, headers=headers, payload=producto)
        log_request_response(url=f"{BASE_URL}{ENDPOINT}", headers=headers, response=response)
...
def crear_producto_metadata_compleja():
    ...
    payload = {
        ...
        "metadata": {"categorias": ["hogar", "decoración"], "detalles": {"peso": "1kg", "color": "rojo"}}
    }
    response = StripeAPI(BASE_URL).post(endpoint=ENDPOINT, headers=headers, payload=payload)
    log_request_response(url=f"{BASE_URL}{ENDPOINT}", headers=headers, response=response)
    return response
...
def crear_producto_campos_adicionales():
    headers = build_headers()
    payload = {"name": "Producto con campos extra", "campo_invalido": "valor inesperado"}
    response = StripeAPI(BASE_URL).post(endpoint=ENDPOINT, headers=headers, payload=payload)
    log_request_response(url=f"{BASE_URL}{ENDPOINT}", headers=headers, response=response)
    return response
